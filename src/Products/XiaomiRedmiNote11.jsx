...
import { redmiNote11 }  from '../Data/dataXiaomi'


function RedmiNote11() {
    ...
    <div>
      <Card  className="text-center" style={{border: "none", background: '#F5F5F5'}}>
        <Carousel  interval='10000' variant = 'dark' activeIndex={index} onSelect={handleSelect} fade='true'>
          ...
          <Card.Body>
            <Link to="/RedmiNote11">{redmiNote11[0].title}</Link>
            <Container>
              <Row className='pt-3 justify-content-md-center'>
                <Col md="auto">
                  <Card.Text>{redmiNote11[0].price + " ₾"}</Card.Text>
                </Col>
              </Row>
              <Row xs={1} md={2}>
