...
import { airPodsPro2 }  from '../Data/dataApple'
    ...
    <div>
      <Card  className="text-center" style={{border: "none", background: '#F5F5F5'}}>
        <Carousel interval='50000' variant = 'dark' activeIndex={index} onSelect={handleSelect} fade='true'>
          ...
          <Card.Body >
            <Link to="/AirPodsPro2">{airPodsPro2[0].title}</Link>
            <Container>
              <Row className='pt-3 justify-content-md-center'>
                <Col md="auto">
                  <Card.Text><s>{airPodsPro2[0].price + " ₾"}</s></Card.Text>
                </Col>
                <Col md="auto">
                  <Card.Text>{airPodsPro2[0].discountPrice + " ₾"}</Card.Text>
                </Col>
              </Row>
              <Row xs={1} md={2}>
