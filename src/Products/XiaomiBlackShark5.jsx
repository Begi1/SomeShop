...
import { blackShark5 }  from '../Data/dataXiaomi'


function BlackShark5() {
    ...
    <div>
      <Card  className="text-center" style={{border: "none", background: '#F5F5F5'}}>
        <Carousel  interval='10000' variant = 'dark' activeIndex={index} onSelect={handleSelect} fade='true'>
          ...
          <Card.Body>
            <Link to="/BlackShark5">{blackShark5[0].title}</Link>
            <Container>
              <Row className='pt-3 justify-content-md-center'>
                <Col md="auto">
                  <Card.Text><s>{blackShark5[0].price + " ₾"}</s></Card.Text>
                </Col>
                <Col md="auto">
                  <Card.Text>{blackShark5[0].discountPrice + " ₾"}</Card.Text>
                </Col>
              </Row>
              <Row xs={1} md={2}>
